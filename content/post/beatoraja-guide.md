...
date: 2021-01-29T16:34:11+08:00
draft: false
---
...
Beatoraja强制需要64位系统。如果你不清楚如何查看64/32位系统的话请自行搜索“查看系统位数”。我根据之前了解的情况，列出了一个表格供大家参考。

/ | 最低配置 | 推荐配置(720p)
---|---|---
CPU性能 | I5-3470 | I5-6500
显卡性能 | GTX650 | GTX960
...
也就是说，目前大部分的电脑都可以流畅运行Beatoraja。极少数的配置不足玩家建议游玩需求配置更低的Lunatic Rave 2。[LR2教程](https://hakula.xyz/tutorial/lr2.html)
...
- 确定[*.bms]文件都在同一个层级下。如 __['选定文件夹'/'分类'/'曲名'/'BMS文件']__。

    - BMS读取的方式是按层级，以上面的格式为例子，如果出现了一个 __['选定文件夹'/'曲名'/'BMS文件']__ 的文件，则下一级的所有文件夹都不会被读取。
...
{{< admonition tip "Tips" false >}}我所用的是Litong5，这个皮肤的设置界面绝大多数都有英文，直接读英文就好。绝大部分的设置和IIDX的设置思路通用。{{< /admonition >}}

- Score Graph是柱状图，实时显示与目标分数/自己最好成绩的差距。
- Judge Timing中打开Fast/Slow显示。
- Notes和Keybeam为note样式和爆炸效果。
- Line亮度为轨道分割线亮度，在Litong5皮肤设置中这条没有英文翻译。
...
按play进入游戏后，确保输入法关闭的情况下点击主键盘6键(不要使用小键盘)，进入案件设置界面。
...
挡板在Litong5内要打开谱面之后在载入时间双击start调整。有些皮肤的start菜单会有Sudden+设置，打开后也在载入时间内调整。调整逻辑和IIDX街机一致。
...
A：建议下载K-lite视频解码包，[官方下载地址](https://www.codecguide.com/download_kl.htm)，一般来说Standard以上的包即可解决。如果还是不能解决的话可以尝试关闭BGA。

Q：选歌界面移动到某一首歌时游戏闪退。

A：说实话我没有发生过此种情况。据了解是由于皮肤中使用了自定义字体导致，建议重新下载皮肤或者更换选歌界面解决。

Q：load difficultly table时卡死。
